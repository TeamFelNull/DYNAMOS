...
public class DynamosModelProvider extends ModelProvider {
    ...
    protected void registerModels(BlockModelGenerators blockModels, ItemModelGenerators itemModels) {
        ...
        for (DynamosIngotEnum ingot : DynamosIngotEnum.values()) {
            int color = ingot.color.getRGB();
            generateColoredItemModel(itemModels, ingot.entry.registeredItem.get(), "ingot_base", color);
            generateColoredItemModel(itemModels, ingot.getNugget().registeredItem.get(), "nugget_base", color);
            generateColoredBlockModel(blockModels,ingot.getBlock().registeredBlock.get(),  "material_block_base", color);
        }
    ...
    public void generateColoredBlockModel(BlockModelGenerators gen, Block block, String textureName, int rgbColor) {
        ...
        ResourceLocation modelLoc = new ModelTemplate(//
                Optional.of(ResourceLocation.fromNamespaceAndPath(Dynamos.MODID, "block/color_block")), // dynamos:block/color_block
        ...
        gen.itemModelOutput.accept(
                block.asItem(),
                ItemModelUtils.tintedModel(modelLoc, ItemModelUtils.constantTint(rgbColor)) // block model を親にした普通の item model
        );
    }
